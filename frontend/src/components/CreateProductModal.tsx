...
export function CreateProductModal({ onProductCreated }: CreateProductModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const headHashBytes = formData.head_hash.startsWith('0x')
                ? formData.head_hash.slice(2).match(/.{1,2}/g)?.map(byte => parseInt(byte, 16)) || []
                : formData.head_hash.match(/.{1,2}/g)?.map(byte => parseInt(byte, 16)) || [];

            await createProduct({
                ...formData,
                head_hash: headHashBytes as any,
            });
    ...
    const updateStage = (index: number, field: 'name' | 'role', value: string) => {
        setFormData(prev => ({
            ...prev,
            [field === 'name' ? 'stage_names' : 'stage_roles']:
                prev[field === 'name' ? 'stage_names' : 'stage_roles'].map((item, i) =>
                    i === index ? value : item
                ),
        }));
    };
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        <Label htmlFor="head_hash">Head Hash (Hex) *</Label>
                        <Input
                            id="head_hash"
                            value={formData.head_hash}
                            onChange={(e) => setFormData(prev => ({ ...prev, head_hash: e.target.value }))}
                            placeholder="0x1234abcd..."
                            required
                        />
                        <p className="text-sm text-muted-foreground mt-1">
                            Enter the initial hash value in hexadecimal format
                        </p>
                    </div>
                                ...
                                <div key={index} className="flex gap-2 items-center">
                                    ...
                                    <div className="flex-1">
                                        <Input
                                            ...
                                            onChange={(e) => updateStage(index, 'role', e.target.value)}
